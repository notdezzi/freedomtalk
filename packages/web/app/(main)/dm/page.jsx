      ...
      <div className="flex-1 flex">
        ...
        <div className="w-60 p-2 border-r border-[var(--border-default)]">
          <div className="space-y-0.5">
            <Link href="/dm" className={cn("flex items-center gap-3 px-2 py-1.5 rounded cursor-pointer", "bg-[var(--bg-modifier-selected)] text-white")}>
        ...
        <div className="flex-1 flex items-center justify-center">
          {channels.length === 0 ? (<div className="text-center max-w-md px-8">
            ...
            </div>) : (<div className="w-full max-w-2xl p-4">
              <h3 className="text-sm font-semibold text-[var(--text-muted)] uppercase mb-2">
                Direct Messages
              </h3>
              <div className="space-y-1">
                {channels.map((channel) => (<Link key={channel.id} href={`/dm/${channel.id}`} className={cn("flex items-center gap-3 p-3 rounded-lg cursor-pointer", "hover:bg-[var(--bg-modifier-hover)] transition-colors")}>
                    <Avatar src={channel.type === "dm" ? channel.recipients[0]?.avatar : channel.icon} alt={channel.type === "dm" ? channel.recipients[0]?.username : channel.name || "Group"} size="md" status={channel.type === "dm" ? "online" : undefined}/>
                    ...
                    : channel.name}
                      ...
                      {channel.lastMessageId && (<div className="text-xs text-[var(--text-muted)] truncate">
