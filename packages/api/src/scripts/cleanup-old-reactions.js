...
async function cleanupOldReactions() {
    ...
    logger.info({ dryRun: DRY_RUN, daysOld: DAYS_OLD, serverId: SERVER_ID }, 'Starting old reactions cleanup');
    try {
        ...
        if (DRY_RUN) {
            ...
            for (let i = 0; i < reactionIds.length; i += BATCH_SIZE) {
                ...
                try {
                    ...
                    logger.error({ error, batch: Math.floor(i / BATCH_SIZE) + 1 }, 'Failed to delete batch of reactions');
                    stats.errors++;
                }
            }
        }
...
cleanupOldReactions()
    ...
    console.log(`Reactions ${DRY_RUN ? 'would be ' : ''}deleted: ${stats.deletedReactions}`);
...
//# sourceMappingURL=cleanup-old-reactions.js.map