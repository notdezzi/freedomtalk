/**
 * DM Channel Integration Tests
 * Tests DM channel functionality with real database
 */
...
describe('DM Channel Integration Tests', () => {
  ...
  afterAll(async () => {
    // Clean up DM channels
    const dmChannels = await db('dm_channels')
      .whereIn('owner_id', [testUserId1, testUserId2, testUserId3])
      .orWhereRaw('id IN (SELECT dm_channel_id FROM dm_channel_participants WHERE user_id IN (?, ?, ?))',
  ...
  describe('createDM', () => {
    it('should create a DM between two users', async () => {
      ...
      const participantIds = dmChannel.participants.map((p: any) => p.user_id);
  ...
  describe('createGroupDM', () => {
    ...
    it('should reject group DM with less than 2 participants', async () => {
      await expect(
        dmChannelService.createGroupDM(testUserId1, [], 'Invalid Group')
      ).rejects.toThrow();
    });
  });
  ...
  describe('getDMsByUser', () => {
    it('should get all DMs for a user', async () => {
      ...
      // All DMs should have the user as a participant
      for (const dm of result.dmChannels) {
        const isParticipant = dm.participants.some((p: any) => p.user_id === testUserId1);
  ...
  describe('addParticipant', () => {
    let groupDMId: string;

    beforeEach(async () => {
      // Create a fresh group DM for each test
      const groupDM = await dmChannelService.createGroupDM(
        testUserId1,
        [testUserId2],
    ...
    it('should add a participant to a group DM', async () => {
      ...
      expect(updated.participants.some((p: any) => p.user_id === newUserId)).toBe(true);
    ...
    it('should reject adding to non-group DM', async () => {
      ...
      await expect(
        dmChannelService.addParticipant(dm.id, testUserId2, testUserId1)
      ).rejects.toThrow();
    });
  });
  ...
  describe('removeParticipant', () => {
    ...
    it('should allow user to remove themselves', async () => {
      const updated = await dmChannelService.removeParticipant(
        groupDMId,
        testUserId3,
        testUserId3
      );

      const activeParticipants = updated.participants.filter((p: any) => p.is_active);
  ...
  describe('updateGroupDM', () => {
    it('should update group DM name', async () => {
      ...
      const updated = await dmChannelService.updateGroupDM(
        groupDM.id,
        { name: 'Updated Name' },
        testUserId1
      );
    ...
    it('should reject update from non-owner', async () => {
      ...
      await expect(
        dmChannelService.updateGroupDM(groupDM.id, { name: 'Hacked Name' }, testUserId2)
      ).rejects.toThrow();
    });
  });
  ...
  describe('deleteDM', () => {
    it('should soft delete DM for user', async () => {
      ...
      // User should no longer be an active participant
      const isParticipant = await dmChannelService.isParticipant(dm.id, testUserId1);
      expect(isParticipant).toBe(false);

      // Other user should still be participant
  ...
  describe('Privacy', () => {
    it('should not allow non-participants to access DM', async () => {
      ...
      // Non-participant trying to access DM details
      const dmChannel = await dmChannelService.getDMById(dm.id);

      // They can get the channel but should not be in participants list
      expect(dmChannel.participants.every((p: any) => p.user_id !== testUserId3)).toBe(true);
    });
  });
});
