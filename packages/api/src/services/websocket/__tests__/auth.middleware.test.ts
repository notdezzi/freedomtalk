...
describe('AuthMiddleware', () => {
  ...
  describe('Error Handling', () => {
    it('should handle unexpected errors gracefully', async () => {
      ...
      // Simulate unexpected error in verifyToken (will be caught by inner try-catch)
    ...
    it('should emit authentication error before calling next', async () => {
      ...
      const emitSpy = vi.spyOn(mockSocket, 'emit' as any);

      await authenticateSocket(mockSocket as Socket, mockNext);

      // Verify emit was called before next
  ...
  describe('Edge Cases', () => {
    it('should handle query token as array (invalid)', async () => {
      mockSocket.handshake!.query = { token: ['token1', 'token2'] as any };
