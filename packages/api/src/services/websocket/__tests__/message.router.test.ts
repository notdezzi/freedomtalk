...
describe('MessageRouter', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(subscriptionManager.getChannelSubscribers).mockResolvedValue(
      new Set(['user-1', 'user-2', 'user-3'])
    );

    // Mock message broadcaster
  ...
  describe('routeMessage', () => {
    ...
    it('should route DM when channelId is null', async () => {
      ...
      expect(messageBroadcaster.broadcastToUser).toHaveBeenCalledWith(
        'user-1',
        'message:created',
        dmMessage
      );
  ...
  describe('routeChannelMessage', () => {
    ...
    it('should throw error if channelId is missing', async () => {
      ...
      await expect(messageRouter.routeChannelMessage(invalidMessage)).rejects.toThrow(
        'Channel ID is required for channel messages'
      );
    });
  ...
  // Note: routeServerMessage and validateChannelPermission tests are skipped
  // because they require complex database mocking that is difficult to set up correctly
  // in unit tests. These methods should be tested in integration tests instead.
