...
describe('RoomManager', () => {
    ...
    describe('joinRoom', () => {
        it('should successfully join a channel room', async () => {
            await roomManager.joinRoom(mockSocket, RoomType.CHANNEL, 'channel-123');
        ...
        it('should successfully join a server room', async () => {
            await roomManager.joinRoom(mockSocket, RoomType.SERVER, 'server-456');
        ...
        it('should successfully join a DM room', async () => {
            await roomManager.joinRoom(mockSocket, RoomType.DM, 'dm-789');
        ...
        it('should throw error when user is not authenticated', async () => {
            mockSocket.data = {};
            await expect(roomManager.joinRoom(mockSocket, RoomType.CHANNEL, 'channel-123')).rejects.toThrow('User not authenticated');
        ...
        it('should throw error when socket.join fails', async () => {
            ...
            await expect(roomManager.joinRoom(mockSocket, RoomType.CHANNEL, 'channel-123')).rejects.toThrow('Socket.io error');
        });
    ...
    describe('Edge Cases and Concurrent Operations', () => {
        ...
        it('should handle leaving room not joined', async () => {
            await roomManager.leaveRoom(mockSocket, RoomType.CHANNEL, 'channel-123');
        ...
        it('should handle concurrent join operations', async () => {
            const promises = [
                roomManager.joinRoom(mockSocket, RoomType.CHANNEL, 'channel-1'),
                roomManager.joinRoom(mockSocket, RoomType.CHANNEL, 'channel-2'),
                roomManager.joinRoom(mockSocket, RoomType.CHANNEL, 'channel-3'),
            ];
        ...
        it('should handle concurrent leave operations', async () => {
            const promises = [
                roomManager.leaveRoom(mockSocket, RoomType.CHANNEL, 'channel-1'),
                roomManager.leaveRoom(mockSocket, RoomType.CHANNEL, 'channel-2'),
                roomManager.leaveRoom(mockSocket, RoomType.CHANNEL, 'channel-3'),
            ];
            await Promise.all(promises);
        ...
        it('should handle room names with special characters', async () => {
            ...
            await roomManager.joinRoom(mockSocket, RoomType.CHANNEL, specialRoomId);
        ...
        it('should handle very long room IDs', async () => {
            ...
            await roomManager.joinRoom(mockSocket, RoomType.CHANNEL, longRoomId);
            expect(mockSocket.join).toHaveBeenCalledWith(`channel:${longRoomId}`);
        });
        it('should handle empty room ID', async () => {
            await roomManager.joinRoom(mockSocket, RoomType.CHANNEL, '');
        ...
        it('should maintain separate membership for different room types with same ID', async () => {
            ...
            await roomManager.joinRoom(mockSocket, RoomType.CHANNEL, roomId);
            await roomManager.joinRoom(mockSocket, RoomType.SERVER, roomId);
            await roomManager.joinRoom(mockSocket, RoomType.DM, roomId);
    ...
    describe('leaveRoom', () => {
        ...
        it('should successfully leave a server room', async () => {
            await roomManager.leaveRoom(mockSocket, RoomType.SERVER, 'server-456');
        ...
        it('should successfully leave a DM room', async () => {
            await roomManager.leaveRoom(mockSocket, RoomType.DM, 'dm-789');
            expect(mockSocket.leave).toHaveBeenCalledWith('dm:dm-789');
            expect(mockRedis.sRem).toHaveBeenCalledWith('room:dm:dm-789', 'user-1');
        });
        it('should throw error when user is not authenticated', async () => {
            mockSocket.data = {};
            await expect(roomManager.leaveRoom(mockSocket, RoomType.CHANNEL, 'channel-123')).rejects.toThrow('User not authenticated');
        ...
        it('should throw error when socket.leave fails', async () => {
            ...
            await expect(roomManager.leaveRoom(mockSocket, RoomType.CHANNEL, 'channel-123')).rejects.toThrow('Socket.io error');
        });
        it('should handle Redis errors gracefully', async () => {
    ...
    describe('getUserRooms', () => {
        it('should return all rooms a user is in', async () => {
            ...
            mockRedis.sIsMember.mockImplementation((key, userId) => {
                if (key === 'room:channel:channel-123' && userId === 'user-1')
                    return Promise.resolve(true);
                if (key === 'room:server:server-456' && userId === 'user-1')
                    return Promise.resolve(true);
