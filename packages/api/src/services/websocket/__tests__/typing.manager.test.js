...
describe('TypingManager', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        vi.useFakeTimers();
        typingManager.lastTypingTime.clear();
        typingManager.typingTimeouts.forEach((timeout) => clearTimeout(timeout));
        typingManager.typingTimeouts.clear();
        mockRedis = {
    ...
    describe('startTyping', () => {
        ...
        it('should broadcast typing start event', async () => {
            await typingManager.startTyping('user-1', 'channel-123');
            expect(roomManager.broadcastToRoom).toHaveBeenCalledWith('channel:channel-123', WS_EVENTS.TYPING_START, expect.objectContaining({
                userId: 'user-1',
    ...
    describe('stopTyping', () => {
        ...
        it('should broadcast typing stop event', async () => {
            await typingManager.stopTyping('user-1', 'channel-123');
            expect(roomManager.broadcastToRoom).toHaveBeenCalledWith('channel:channel-123', WS_EVENTS.TYPING_STOP, expect.objectContaining({
                userId: 'user-1',
                channelId: 'channel-123',
                timestamp: expect.any(String),
            }));
...
//# sourceMappingURL=typing.manager.test.js.map